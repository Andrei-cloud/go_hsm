...
// AllocBufferFromPool allocates guest memory via the wasm Alloc export and writes
// the given host-side data from a pooled buffer into the guest's linear memory, returning the pointer address.
// This is useful when processing large batches of similar-sized requests.
func AllocBufferFromPool(
	ctx context.Context,
	mod api.Module,
	alloc api.Function,
	data []byte,
